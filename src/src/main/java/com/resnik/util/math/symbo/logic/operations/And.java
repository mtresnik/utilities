...
public class And<T extends LogicalOperation> extends LogicalOperation {


    public And(T o1, T o2, T ... o_n){
    ...
    public LogicalConstant constantRepresentation() {
        for(LogicalOperation operation : this.values){
            if(LogicalConstant.FALSE.equals(operation.constantRepresentation())){
    ...
    @Override
    public And generate(LogicalOperation[] ops) {
        return new And(ops);
    }
