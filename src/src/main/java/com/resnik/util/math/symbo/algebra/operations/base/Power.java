...
import com.resnik.util.math.symbo.algebra.operations.functions.Log;
...
public class Power extends Operation<Operation> {
    ...
    public String nonConstantString() {
        if (exponent instanceof Constant || exponent.allConstants()) {
            ...
            if (e_c.getValue().equals(ComplexNumber.ONE)) {
                return base.toString();
            }
            if (e_c.getValue().equals(ComplexNumber.ZERO) && base instanceof Vector == false) {
    ...
    public Operation getDerivative(Variable dVar) {
        ...
        if (g1x instanceof Constant) {
            ...
            if (g1x_c.getValue().equals(ComplexNumber.ZERO)) {
                return right;
            }
        }
        if (f1x instanceof Constant) {
            Constant f1x_c = (Constant) f1x;
            if (f1x_c.getValue().equals(ComplexNumber.ZERO)) {
                return left;
            }
        }
    ...
    public Constant constantRepresentation() {
        ...
        ComplexNumber c1 = base.constantRepresentation().getValue().pow(exponent.constantRepresentation().getValue());
    ...
    public Operation getIntegral(Variable dVar) {
        if(base.equals(dVar) && exponent instanceof Constant){
            ComplexNumber oldConst = exponent.constantRepresentation().getValue();
